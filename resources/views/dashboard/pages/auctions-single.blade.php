...
<body>
    ...
    <main id="main" class="main">
        ...
        <div class="bg-trans p-2">
            @if ($currentUser->can('create-lots') && $auction->status == -1)
                @if ($auction->status == -1)
            ...
            @if ($currentUser->sellsHorses())

                @if (\Carbon\Carbon::now()->between($auction->entry_start_datetime, $auction->entry_end_datetime))
                    ...
                    <div class="modal fade" id="ExtralargeModal">
                        <div class="modal-dialog modal-xl">
                            <form method="post" action="{{ url('operations/auction/lots/add-reg-request') }}">
                                {{ csrf_field() }}
            ...
            @if ($currentUser->can('edit-auctions'))
                ...
                <a href="{{ url('tv-banner/' . $auction->id) }}" target="_blank" class="btn btn-light m-1" style="float: right">
                    <i class="fa fa-tv"></i> Tv Banner
                </a>
                <a href="{{ url('tv-banner-and-results/' . $auction->id) }}" target="_blank" class="btn btn-light m-1" style="float: right">
                    <i class="fa fa-list"></i> Bids & Tv Banner
                </a>
                <a href="{{ url('hall-bidders/' . $auction->id) }}" class="btn btn-light m-1" style="float: right">
                    <i class="fa fa-users"></i> Hall bidders
                </a>
                @if ($auction->status == 1)
                    <a href="{{ url('auction-report/' . $auction->id) }}" class="btn btn-light m-1"
                        style="float: right">
                        <i class="fa fa-trophy"></i> Winners
                    </a>

                    <a href="{{ url('operations/auction-log-generator/' . $auction->id) }}" class="btn btn-light m-1"
                        style="float: right">
            ...
            <div class="pagetitle">
                <form method="post" id="publishMaster" action="{{ url('operations/auction/publish') }}">
                    {{ csrf_field() }}
                    <input type="hidden" name="auctionId" value="{{ $auction->id }}" />
                </form>
        ...
        <section class="section dashboard ">
            ...
            <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    @if ($auction->status == -1)
                        @if (!$currentUser->can('accept-reject-horses'))
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <a style="font-size: 12px"
                                                href="{{ url('horse-details/' . $horse->id) }}?redirected=auction&auction_id={{ $auction->id }}">More
                                                details</a>
                                        ...
                                        <td><?= $req->notes ?> <?php if($req->is_pregnant){ ?>
                                            , Pregnant , From {{ $req->pregnant_from }}
                                            , Due date {{ $req->pregnant_due_date }}
                                            <?php } ?>
                                        </td>
                  ...
                  ?>
                                    <tr>
                                        ...
                                        <td>
                                            <form method="post"
                                                action="{{ url('operations/auction/accept-lot-join-request') }}">
                                                {{ csrf_field() }}
                      ...
                      ?>
                        ...
                        <div class="text-center p-5">
                            ...
                            <h3>Not lots approved yet
                                @if ($currentUser->can('create-lots'))
              ...
              foreach ($all as $a){
                 ...
                 if(count($all) > 0) {
                  ...
                  ?>
                        ...
                        <p style="font-weight: bold;">{{ ucfirst($status) }} lots</p>


                        @if ($currentUser->can('start-stop-lot') && $status == 'offline')
    ...
    ">
                            ...
                            ?>
                                    <option {{ $a->status_string == 'sold' ? 'disabled' : '' }}
                                        value="{{ $a->id }}">Lot# {{ $a->order_sn + 1 }}
                                        {{ $a->horse->name_en }} ({{ $a->status_string }})</option>
                                    <?php } ?>
                                </select>
                        ...
                        @if (
                            ...
                            <form method="post" action="{{ url('operations/auction/start-lot-manually') }}">
                                {{ csrf_field() }}
    ...
    ">
                            ...
                            ?>
                                    <option {{ $a->status_string != 'created' ? 'disabled' : '' }}
                                        value="{{ $a->id }}">Lot# {{ $a->order_sn + 1 }}
                                        {{ $a->horse->name_en }} ({{ $a->status_string }})</option>
                                    <?php } ?>
                                </select>
         ...
         foreach ($all as $item){
           ...
           if ($item->status_string == 'created') {
             ...
             ?>
                            <tr
                                class="lot-{{ $item->status_string }} {{ $item->status_string == 'started' ? 'blk-bg' : '' }}  ">
                                ...
                                <td><small><?= $item->target_type == 'breeding-right' ? ($item->horse->gender == 'mare' ? 'Embryo of : ' : 'Breeding right from : ') : '' ?>
                                    ...
                                    <a style="font-size: 12px"
                                        href="{{ url('horse-details/' . $item->horse_id) }}?redirected=auction&auction_id={{ $auction->id }}">More
                                        details</a>

                                    <?php if($item->is_pregnant){ ?>
                                    , In Foal , From {{ $item->pregnant_from }}
                                    , Due date {{ $item->pregnant_due_date }}
                                    <?php } ?>
    ...
    margin-right: 5px;"
                                ...
                                <td>
                                    ...
                                    <a href="{{ url('lot/' . $item->id) }}" class="btn btn-success btn-sm btn-block"><i
                                            class="fa fa-trophy"></i> Results</a>
                                    <?php } ?>


                                    <?php if($item->status_string == 'started'){  ?>
                                    <a href="{{ url('lot/' . $item->id) }}"
                                    ...
                                    <form method="post" action="{{ url('operations/auction/start-silent-lot') }}">
                                        {{ csrf_field() }}
                                    ...
                                    <form method="post" action="{{ url('operations/auction/pause-silent-lot') }}">
                                        {{ csrf_field() }}
                                        <input type="hidden" name="lot_id" value="{{ $item->id }}" />
                                        <button type="submit"
         ...
         ?>
                ...
                <div class="col-md-4">
                    ...
                    <div class="card  p-2">
                        <img class="responsive-img" style="width: 100%" src="{{ $auction->buildPoster() }}" />
                                    ...
                                    <div class="list-group-item d-flex align-items-center justify-content-between"
                                        ...
                                        <div>
                                            <p class="mb-0 d-inline-flex align-items-center">
                                                {{ $item->order_sn + 1 }}) {{ $item->name_en }} ({{ $item->lot_type }})
                                            </p>
                                        </div>
                                    </div>
                                    <?php } ?>
    ...
    text-align: center;">
        ...
        @if ($currentUser->can('create-lots'))
            <div class="modal fade" id="ExtralargeModal">
                <div class="modal-dialog modal-xl">
                    <form method="post" action="{{ url('operations/auction/lots/add-bulk') }}" id="identifier">
                        {{ csrf_field() }}
                        <div class="modal-content">
                            <div class="modal-header">
        ...
        @if ($currentUser->can('edit-auctions'))
            ...
            <div class="modal fade" id="ExtralargeModalSettings">
                <div class="modal-dialog modal-xl">
                    <form method="post" action="{{ url('operations/auction/edit') }}" onsubmit="populateQuiil()"
    ...
    background-color: #fff6f3;">
                                                <div class="row row g-3">
                                                    <div class="col-md-6">
                                                        ...
                                                        <input type="date" class="form-control"
                                                            {{ $live == 0 ? '' : 'disabled' }} name="start_date"
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <input type="number" name="lot_duration"
                                                            {{ $live == 0 ? '' : 'disabled' }} class="form-control"
                                                            placeholder="Lot duration in minutes"
                                                            value="{{ $auction->lot_duration }}">
                                                    </div>
                                                    <div class="col-6">
                                                        ...
                                                        <input type="number" name="auction_interval"
                                                            ...
                                                            value="{{ $auction->auction_interval }}">
                                                    </div>
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <input type="number" name="remind_start_before"
                                                            ...
                                                            value="{{ $auction->remind_start_before }}">
                                                    </div>
                                                    <div class="col-6">
                                                        ...
                                                        <input type="number" name="remind_end_before"
                                                            ...
                                                            value="{{ $auction->remind_end_before }}">
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div id="editor" style="       max-height: 25rem;">{!! $auction->terms !!}
                                        </div>

                                        <textarea id="hiddenArea" style="display: none" name="terms">{!! $auction->terms !!}</textarea>
                                    </div>
    ...
    <button class=\"btn btn-danger btn-sm btn-remove-bid-button\" data-value='" + json[i].value +
        ...
        @if ($currentUser->can('edit-lots')) @if ($cnt > 0)   
       @if ($live == 0)
